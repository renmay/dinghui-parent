    ...
    <div class="container">
      ...
      <div class="mgt25">
        ...
        <div class="fr list-right bgf innk bdr4" style="height:357px;overflow: hidden;">
          ...
          <ul class="content-list" v-for="item in entranceList">
            <li class="tof"><i></i>
              <router-link to="/inquire">{{item.title}}</router-link>
            </li>
          </ul>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getBannerList() {
        this.$http.get('/api/schoolpublicimag/findall?type=1').then(res => {  //这是从本地请求的数据接口，
          this.bannerList = res.body
        })
      },
      getEntranceList() {
        this.$http.get('/api/schoolpublicimag/findall?type=2').then(res => {  //这是从本地请求的数据接口，
          this.entranceList = res.body
          console.log(this.entranceList)
        })
      },
      getSchoolListList() {
        this.$http.get('/api/schoolpublicimag/findall?type=3').then(res => {  //这是从本地请求的数据接口，
          this.schoolList = res.body.list
...
<style scoped>
  ...
  .text-ball .pinform {
    background: url("/static/img/inform.png") no-repeat 7px 7px;
  }

  .text-ball .xgzc {
    background: url("/static/img/xgzc.png") no-repeat 7px 9px;
  }
  ...
  .mssege i {
    width: 60px;
    height: 40px;
    background: url("/static/img/xx.png") no-repeat 10px 3px;
  }

  .school i {
    width: 60px;
    height: 40px;
    background: url("/static/img/xgzc2.png") no-repeat 10px 3px;
  }
  ...
  .content-list li > i {
    width: 35px;
    height: 40px;
    background: url("/static/img/tree.png") no-repeat 0px 3px;
